...
# 判断是否在主目录
if not Path("vite.config.js").is_file():
    print("请在项目主目录下运行此脚本")
    exit(1)
...
def generator(api_data, api_index):
    ...
    lines.append("```json")
    lines.extend(
        json.dumps(api_data["response"], indent=2, ensure_ascii=False).splitlines()
    )
    lines.append("```")
    lines.append("**失败**:")
    lines.append("```json")
    lines.extend(
        json.dumps(api_data["fail"], indent=2, ensure_ascii=False).splitlines()
    )
    lines.append("```")
    lines.append("")
...
files_dir = Path(__file__).parent.absolute()

# glob all json files in the directory
json_files = [f for f in files_dir.iterdir() if f.is_file() and f.suffix == ".json"]
data_list = []
...
# write to file
output_path = Path("dist") / "接口文档.auto.md"
