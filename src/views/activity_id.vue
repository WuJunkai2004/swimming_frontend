...
const callPlayerDialog = (hash) => {
  if (hash && hash.startsWith("#playerId=") && playerDialogRef.value) {
...
watch(
  () => route.hash,
  (newHash) => {
    callPlayerDialog(newHash);
  },
);
watch(
  () => isDetailDialogVisible.value,
  (visible) => {
    if (!visible && route.hash.startsWith("#playerId=")) {
      console.log("关闭运动员详情弹窗，清除 URL hash");
      router.replace({ hash: "" });
    }
  }
);
        ...
        <div class="content-body">
          <template v-for="(item, idx) in newsDetail.content" :key="idx">
            ...
            <div
              ...
              class="flex flex-column align-items-center my-4"
            >
              <Image
                :src="item.url"
                class="news-image"
            ...
            <div
              ...
              class="news-video flex justify-content-center my-4"
            >
              <video
                :src="item.url"
                controls
